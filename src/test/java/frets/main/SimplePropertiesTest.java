...
public class SimplePropertiesTest {
	...
	public void testMock1() {
		SimpleProperties<MockProperties> mock = 
		   MockProperties.instance.getInstance("Mock properties name 1");
		...
		String expected = "src" + PATH + "test" + PATH + "resources" + PATH + "frets" + PATH + "main" + PATH + "mock1.properties";
	...
	public void testMock2() {
		SimpleProperties<MockProperties> mock = 
			MockProperties.instance.getInstance("Mock properties name 2");
		...
		String expected = "src" + PATH + "test" + PATH + "resources" + PATH + "frets" + PATH + "main" + PATH + "mock2.properties";
		assertEquals("Location", expected, mock.getMetaLocation());
	}
	...
	public static class MockProperties implements SimpleProperties<MockProperties> {
		...
		protected static Map<String, MockProperties> propertiesMap; 
		// static collection for all these objects.
		...
		/**
		 * Opens a file at the given name, and reads all the properties into an
		 * object.
		 */
		public MockProperties readFromFile(String fileName) throws IOException {
			...
			// test.exists() + ", abspath=" + test.getAbsolutePath());
			...
			for (Iterator<Object> it = properties.keySet().iterator(); it.hasNext();) {
				...
				if ("name".equals(key)) {
					...
					throw new IllegalArgumentException("Class=\"" + MockProperties.class.getName()
							+ "\" key \"" + key + "\" not handled, value=\"" + value + "\".");
				}
			}
			return mock;
		}
		 ...
		 */
		public List<MockProperties> readFromPath(String pathName, String filterString)
				throws IOException {
		...
		/**
		 * Returns an object fretboard that has been loaded from a central
		 * repository or properties list.
		 */
		public MockProperties getInstance(String shortName) {
			if (null == propertiesMap) {
				// Lazy instantiation
				try {
					propertiesMap = loadProperties("src/test/resources/frets/main/",
							"mock.*.properties");
				} catch (IOException e) {
		...
		public Map<String, MockProperties> loadProperties(String pathName, String filterString)
				throws IOException {
